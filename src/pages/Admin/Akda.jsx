...
function Akda() {
  ...
  const [akdaDates, setAkdaDates] = useState({});
  ...
  const fetchAkdaDates = async () => {
    try {
      ...
      setError("Failed to fetch Akda dates");
      setLoading(false);
    }
  };
  ...
  const renderMonthList = () => {
    ...
    return Object.keys(akdaDates[selectedYear])
      .sort((a, b) => b - a)
      .map((month) => (
        <button
          key={month}
  ...
  const renderDateList = () => {
    return akdaDates[selectedYear][selectedMonth]
      .sort((a, b) => b - a)
      .map((date) => (
        <Link
