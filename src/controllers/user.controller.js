...
const addDukaandar = asyncHandler(async (req, res) => {
  ...
  return res.json(
    new ApiResponse(200, "Dukaandar created successfully", dukaandar)
  );
});
...
const viewDukaandars = asyncHandler(async (req, res) => {
  ...
  return res.json(
    new ApiResponse(200, "Dukaandar found successfully", dukaandars)
  );
});
...
const viewDukaandarById = asyncHandler(async (req, res) => {
  ...
  return res.json(
    new ApiResponse(200, "Dukaandar found successfully", dukaandar)
  );
});
...
const pattiGeneration = asyncHandler(async (req, res) => {
  const dukaandarBalance = await Dukaandar.find({})
    .select("name balance")
    .sort({ balance: -1 });
  const bepariBalance = await Bepari.find({})
    .select("name balance")
    .sort({ balance: -1 });

  if (!dukaandarBalance || !bepariBalance) {
    throw new ApiError(404, "Dukaandar or Bepari not found");
  }

  return res.json(
    new ApiResponse(200, "Dukaandar and Bepari found successfully", {
      dukaandarBalance,
      bepariBalance,
    })
  );
